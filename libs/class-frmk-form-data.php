...
class FRMK_FormData {
				...
				if ( isset( $upload_data[ $field->get_input_name() ] ) ) {
					...
					if ( isset( $upload_data[ $field->get_input_name() ]['name'] ) && isset( $upload_data[ $field->get_input_name() ]['error'] ) ) {
						...
						if ( 0 === $upload_data[ $field->get_input_name() ]['error']
							...
							$upload_dir = frmk_get_uploads_dir();
		...
		foreach ($submission_data as $data){
			...
			switch ( $type ) {
				...
				case 'number':
					...
					break;
				default:
					$this->_fields[ $field_id ] = new FRMK_FormField( $field_id, $type, $field );
					break;
			}
	...
	public function get( $field_id ) {
		...
		$result = apply_filters( 'frmk/field_data', $result, $field_id );
	...
	public function get_field( $field_id ) {
		...
		if ( ! $found ){
			return new FRMK_FormField($field_id, 'virtual');
		}
